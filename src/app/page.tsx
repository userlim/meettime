...
function CalendarPicker({ selectedDates, onToggleDate, locale }: { selectedDates: string[], onToggleDate: (d: string) => void, locale: Locale }) {
  ...
  return (
    <div className="bg-white/[0.03] rounded-xl border border-[rgba(255,255,255,0.06)] p-4">
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setViewDate(new Date(year, month - 1, 1))} className="p-2 hover:bg-white/[0.04] rounded-lg transition text-[var(--text-muted)]" aria-label="Previous">
        ...
        <button onClick={() => setViewDate(new Date(year, month + 1, 1))} className="p-2 hover:bg-white/[0.04] rounded-lg transition text-[var(--text-muted)]" aria-label="Next">
      ...
      <div className="grid grid-cols-7 gap-1 text-center mb-2">
        {days.map(d => <div key={d} className="text-xs font-medium text-[var(--text-muted)] py-1">{d}</div>)}
      </div>
          ...
          return (
            <button key={dateStr} disabled={isPast} onClick={() => !isPast && onToggleDate(dateStr)}
              className={`w-full aspect-square flex items-center justify-center text-sm font-medium rounded-lg transition
                ${isPast ? 'text-[var(--text-muted)] cursor-not-allowed' : isSelected ? 'bg-indigo-600 text-white' : 'text-[var(--text-muted)] hover:bg-indigo-500/10 cursor-pointer'}`}
            >{day}</button>
          )
        })}
      </div>
    ...
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[var(--text-secondary)] text-base sm:text-lg max-w-lg mx-auto">{t(locale, 'heroDesc')}</p>
        </div>

        <div className="bg-white/[0.03] rounded-2xl shadow-sm border border-white/5 p-6 sm:p-8 mb-8">
          <div className="mb-6">
            <label className="block text-sm font-semibold text-[var(--text-muted)] mb-2">{t(locale, 'eventName')}</label>
            <input type="text" value={eventName} onChange={e => setEventName(e.target.value)}
              placeholder={t(locale, 'eventNamePlaceholder')}
              className="w-full px-4 py-3 border border-[rgba(255,255,255,0.06)] rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-gray-200 bg-white/[0.03]" maxLength={100} />
          </div>
          ...
          <div className="mb-6 grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-[var(--text-muted)] mb-2">{t(locale, 'earliestTime')}</label>
              <select value={startHour} onChange={e => setStartHour(parseInt(e.target.value))}
                className="w-full px-4 py-3 border border-[rgba(255,255,255,0.06)] rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white/[0.03] text-gray-200">
                {hours.map(h => <option key={h} value={h} disabled={h >= endHour}>{formatHour(h)}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-[var(--text-muted)] mb-2">{t(locale, 'latestTime')}</label>
          ...
          <div className="mb-8">
            <label className="block text-sm font-semibold text-[var(--text-muted)] mb-2">{t(locale, 'timezone')}</label>
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {['step1','step2','step3'].map((step, i) => (
              <div key={step} className="text-center p-4">
                ...
                <p className="text-sm text-[var(--text-secondary)]">{t(locale, `${step}Desc`)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-12 mb-8">
          ...
          <div className="space-y-3">
            {['Free','Account','Timezone','Limit','Mobile','Heatmap'].map(key => (
              <details key={key} className="bg-white/[0.03] border border-[rgba(255,255,255,0.06)] rounded-xl p-4 group">
                <summary className="font-medium text-gray-200 cursor-pointer list-none flex items-center justify-between">
                  {t(locale, `faq${key}Q`)}
                  <svg className="w-5 h-5 text-[var(--text-muted)] group-open:rotate-180 transition-transform shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>
                </summary>
                <p className="mt-3 text-[var(--text-muted)] text-sm leading-relaxed">{t(locale, `faq${key}A`)}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section id="content-depth-section" className="mt-20 max-w-3xl mx-auto px-6 relative z-[1]">
        <h2 className="text-h2 mb-6 text-[var(--text-primary)]">Complete Guide</h2>
            ...
            <div className="mb-6">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">The Challenge of Global Meeting Scheduling</h3>
            ...
            <div className="mb-6">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">How MeetTime Solves Time Zone Coordination</h3>
            ...
            <div className="mb-6">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">Best Practices for International Meeting Scheduling</h3>
            ...
            <div className="mb-6">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">Understanding Daylight Saving Time Complexities</h3>
            ...
            <div className="mb-6">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">The Future of Global Work Scheduling</h3>
      ...
      <section id="faq-section" className="mt-12 max-w-3xl mx-auto px-4">
        <h2 className="text-h2 mb-6 text-[var(--text-primary)]">Frequently Asked Questions</h2>
        
            <div className="mb-4">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">How does MeetTime help schedule meetings across time zones?</h3>
              <p className="text-[var(--text-secondary)]">MeetTime lets you overlay multiple time zones on a visual timeline. Select participants' time zones and see everyone's business hours at a glance, making it easy to find a time that works for all attendees.</p>
            </div>
            <div className="mb-4">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">Can I share my MeetTime schedule with others?</h3>
              <p className="text-[var(--text-secondary)]">Yes! Create your availability, then share a unique link with participants. They can view time options in their own local time zone and mark their availability. No account needed.</p>
            </div>
            <div className="mb-4">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">How many time zones can I compare at once?</h3>
              <p className="text-[var(--text-secondary)]">MeetTime supports comparing unlimited time zones simultaneously. Whether you're coordinating between 2 cities or 10, the visual timeline makes it easy to find overlapping availability.</p>
            </div>
            <div className="mb-4">
              <h3 className="text-h3 mb-2 text-[var(--text-primary)]">Is MeetTime free to use?</h3>
              <p className="text-[var(--text-secondary)]">Yes, MeetTime is completely free with no signup required. Create meetings, share links, and coordinate schedules across time zones at no cost.</p>
            </div>
      </section>
